...
export const DIAGRAM_COMPOSITION_GUIDANCE =
  ...
  + "完整分类树可用 hierarchy + composition: atlas，把同一输入分成数量总览和完整明细；仅支持一个根、分类、末级条目三级，不得删除交叉关系来套用。"
...
export const CANVAS_DIAGRAM_SKILL: SkillRegistration = {
  ...
  content: [
    ...
    + "输入互相矛盾时保留冲突，不自行裁决。",
    "   - `hierarchy` 可显式选 `composition: atlas`：先看分类与条目数量，再查完整名称与说明。仅用于无分组、无多父节点、无循环的根→分类→条目树；有跨层引用、额外层级或分组时保留原图型，不为套版式删除关系。",
    "   - atlas 的两层共享同一份节点和边；不要手写节点计数，不要把模型适用对象、引用关系或因果关系当成分类。摘要说明来源与本图边界。",
